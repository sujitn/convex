...
impl Sovereign {
    ...
    ///
    /// Note: Some currencies are not yet in convex-core, so we use the most
    /// common reference currency (USD for Americas/Middle East, EUR for
    /// Eastern Europe).
    #[must_use]
    pub fn currency(&self) -> Currency {
        match self {
            ...
            // Latin American: use USD (common for external issuance)
            ...
            // Eastern Europe: use EUR (reference currency)
    ...
    pub fn standard_tenors(&self) -> &'static [u32] {
        match self {
            ...
            // UK: 2Y, 5Y, 10Y, 30Y, 50Y
            Self::UK => &[24, 60, 120, 360, 600],
            // Germany: 2Y (Schatz), 5Y (Bobl), 10Y (Bund), 30Y
            Self::Germany => &[24, 60, 120, 360],
            // Japan: 2Y, 5Y, 10Y, 20Y, 30Y, 40Y
    ...
    /// (e.g., some EM sovereigns issue USD-denominated bonds).
    #[must_use]
...
mod tests {
    ...
    fn test_standard_tenors() {
        ...
        assert!(ust_tenors.contains(&24)); // 2Y
        assert!(ust_tenors.contains(&120)); // 10Y
        assert!(ust_tenors.contains(&360)); // 30Y
    }
