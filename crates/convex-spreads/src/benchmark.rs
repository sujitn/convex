...
///
/// ```rust,ignore
/// // Most common: interpolate at bond's exact maturity
/// let spec = BenchmarkSpec::interpolated();
///
/// // Spread to 10-year on-the-run
/// let spec = BenchmarkSpec::ten_year();
///
/// // Spread to specific security by CUSIP
/// let spec = BenchmarkSpec::cusip("91282CJN6")?;
///
...
pub enum BenchmarkSpec {
    ...
    ///
    /// Example: "spread to 10-year" for all bonds, even if they have
    /// different maturities.
    OnTheRunTenor(Tenor),

    /// Use the on-the-run benchmark closest to bond's maturity.
    ///
    /// Rounds to nearest standard tenor: 2Y, 3Y, 5Y, 7Y, 10Y, 20Y, 30Y.
    NearestOnTheRun,

    /// Use a specific treasury identified by CUSIP, ISIN, or FIGI.
    ///
    /// For precise benchmark selection when the exact security matters.
    SpecificSecurity(SecurityId),

    /// Use an explicitly provided benchmark yield.
    ///
    /// For cases where the caller already has the benchmark yield
    /// (e.g., from a live data feed) or wants to use a non-standard
    /// benchmark.
    ExplicitYield(Yield),
}
