...
pub struct AggregateBookRiskParams {
    /// Positions to aggregate. Single-currency, single-evaluation-date.
    pub positions: Vec<ComputePositionRiskParams>,
    /// Becomes `position_id` on the aggregate.
    #[serde(default)]
...
impl ConvexMcpServer {
    ...
    #[tool(
        description = "Book-level risk: prices each position, sums DV01, sums \
        KRD bucket-by-bucket, DV01-weights durations and convexity, \
        face-weights per-100 prices, zeroes `ytm_decimal`. Returns a single \
        `RiskProfile` with `position_id = book_id`, ready to feed into \
        `propose_hedges`. Rejects mismatched currency, evaluation date, or \
        KRD ladder across positions."
    )]
    ...
    /// Shared compute body — routes OAS marks on callables through the
    /// HW1F path, everything else through the generic.
...
mod tests {
    ...
    async fn aggregate_book_risk_routes_oas_position_through_callable_path() {
        // Exercises the OAS branch of the factored `position_risk` helper.
        let server = ConvexMcpServer::new();
