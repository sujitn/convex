...
pub unsafe extern "C" fn convex_curve_zero_rate(handle: Handle, tenor: c_double) -> c_double {
    registry::with_object::<StoredCurve, _, _>(handle, |curve| {
        curve
            .zero_rate_at_tenor(tenor, Compounding::Continuous)
            .unwrap_or(f64::NAN)
    })
    .unwrap_or(f64::NAN)
}
...
pub unsafe extern "C" fn convex_curve_forward_rate(
    handle: Handle,
    start_tenor: c_double,
    end_tenor: c_double,
) -> c_double {
    registry::with_object::<StoredCurve, _, _>(handle, |curve| {
        curve
            .forward_rate_at_tenors(start_tenor, end_tenor, Compounding::Continuous)
            .unwrap_or(f64::NAN)
    })
    .unwrap_or(f64::NAN)
}
...
pub unsafe extern "C" fn convex_bootstrap_from_instruments(
    ...
    ref_month: c_int,
    ref_day: c_int,
    deposit_tenors: *const c_double,
    deposit_rates: *const c_double,
    deposit_count: c_int,
    swap_tenors: *const c_double,
    swap_rates: *const c_double,
    swap_count: c_int,
    _interpolation: c_int,
    day_count: c_int,
) -> Handle {
    let ref_date = match Date::from_ymd(ref_year, ref_month as u32, ref_day as u32) {
        Ok(d) => d,
