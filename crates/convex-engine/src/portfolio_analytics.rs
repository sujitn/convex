...
impl PortfolioAnalyzer {
    ...
    pub fn duration_contribution(
        ...
        for position in &portfolio.positions {
            if let Some(quote) = price_map.get(&position.instrument_id) {
                if let (Some(dirty_price), Some(dur)) = (quote.dirty_price, quote.modified_duration)
                {
                    let mv = position.notional * dirty_price / Decimal::from(100);
                    total_mv += mv;
    ...
    fn create_test_quotes() -> Vec<BondQuoteOutput> {
        ...
        vec![
            ...
            BondQuoteOutput {
                ...
                currency: Currency::USD,
                settlement_date: date(2025, 6, 17),
                clean_price: Some(dec!(98.00)),
                dirty_price: Some(dec!(99.00)),
                accrued_interest: Some(dec!(1.00)),
                ytm: Some(dec!(0.055)),
                ytw: None,
                ytc: None,
                z_spread: Some(dec!(120)),
                i_spread: None,
                g_spread: None,
                asw: None,
                oas: None,
