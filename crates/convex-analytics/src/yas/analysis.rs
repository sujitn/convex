...
impl YasAnalysisBuilder {
    ...
    pub fn build(self) -> Result<YasAnalysis, AnalyticsError> {
        Ok(YasAnalysis {
            street_convention: self.street_convention.ok_or_else(|| {
                AnalyticsError::InvalidInput("street_convention is required".to_string())
            })?,
            ...
            invoice: self.invoice.ok_or_else(|| {
                AnalyticsError::InvalidInput("settlement invoice is required".to_string())
            })?,
        })
    }
}
...
mod tests {
    ...
    #[test]
    fn test_yas_builder_missing_required() {
        let result = YasAnalysis::builder().build();
