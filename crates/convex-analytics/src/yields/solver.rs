...
//!
//! ```rust,ignore
//! use convex_analytics::yields::{YieldSolver, YieldResult};
...
use convex_bonds::traits::{BondCashFlow, FixedCouponBond};
use convex_bonds::types::YieldConvention;
...
impl YieldSolver {
    ...
    fn solve_with_brent<F>(&self, objective: F, initial_guess: f64) -> AnalyticsResult<YieldResult>
    where
        F: Fn(f64) -> f64,
    {
...
pub fn current_yield_from_fixed_bond(bond: &dyn FixedCouponBond, clean_price: Decimal) -> f64 {
    ...
    current_yield_decimal(annual_coupon, clean_price)
}
...
mod tests {
    ...
    fn test_current_yield() {
        ...
        let cy = current_yield_decimal(annual_coupon, clean_price);
