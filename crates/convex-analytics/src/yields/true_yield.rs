...
mod tests {
    ...
    #[test]
    fn test_true_yield_positive_adjustment() {
        let street = dec!(0.05);
        let adj = dec!(0.0002); // +2bp adjustment

        let true_y = true_yield(street, adj);
        assert_eq!(true_y, dec!(0.0502));
    }

    #[test]
    fn test_settlement_adjustment_default() {
        let adj = settlement_adjustment(2, 0.05);
        assert_eq!(adj, Decimal::ZERO);
    }
}
