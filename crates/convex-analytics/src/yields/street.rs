...
fn newton_raphson_yield(
    ...
    for iteration in 0..MAX_ITERATIONS {
        ...
        if dpv.abs() < 1e-15 {
            return Err(AnalyticsError::SolverConvergenceFailed {
                solver: "derivative too small".to_string(),
    ...
    Err(AnalyticsError::SolverConvergenceFailed {
        solver: "newton raphson".to_string(),
...
mod tests {
    ...
    fn test_street_convention_mismatched_lengths() {
        ...
        let result = street_convention_yield(100.0, &cash_flows, &times, 2, 0.05);
        assert!(result.is_err());
    }

    #[test]
    fn test_street_convention_empty_cash_flows() {
        let result = street_convention_yield(100.0, &[], &[], 2, 0.05);
        assert!(result.is_err());
    }
}
