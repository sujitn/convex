...
impl Schedule {
    ...
    pub fn generate(config: ScheduleConfig) -> AnalyticsResult<Self> {
        if config.end_date <= config.start_date {
            return Err(AnalyticsError::ScheduleGenerationFailed(
    ...
    fn generate_backward(
        ...
        while current > config.start_date {
            current = current.add_months(-months_per_period).map_err(|e| {
                AnalyticsError::ScheduleGenerationFailed(format!("Date arithmetic error: {e}"))
            })?;
    ...
    fn adjust_dates(dates: &[Date], config: &ScheduleConfig) -> AnalyticsResult<Vec<Date>> {
        let calendar = calendar_id_to_calendar(&config.calendar);
        ...
        dates
            ...
            .map(|&date| {
                calendar
                    ...
                    .map_err(|e| {
                        AnalyticsError::ScheduleGenerationFailed(format!(
                            "Failed to adjust date {date}: {e}"
                        ))
                    })
            })
            .collect()
...
/// Converts a CalendarId to a boxed Calendar trait object.
#[must_use]
