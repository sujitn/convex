...
/// Stub type indicating whether a period is short or long.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum IrregularStubType {
    /// No stub (regular period)
    #[default]
    None,
    /// Short stub (period is shorter than regular)
    ShortStub,
    /// Long stub (period is longer than regular)
    LongStub,
}

/// Reference period calculation method for irregular periods.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum ReferenceMethod {
    /// ICMA methodology (standard for most markets)
    #[default]
    ICMA,
    /// ISDA methodology
    ISDA,
...
impl IrregularPeriodHandler {
    ...
    pub fn first_period_stub_type(
        ...
        if (actual_days - regular_days).abs() <= 5 {
            IrregularStubType::None
        } else if actual_days < regular_days {
            IrregularStubType::ShortStub
        } else {
            IrregularStubType::LongStub
        }
    }
...
mod tests {
    ...
    fn test_stub_type_short() {
        ...
        assert_eq!(stub_type, IrregularStubType::ShortStub);
    }
    ...
    fn test_stub_type_long() {
        ...
        let stub_type =
            IrregularPeriodHandler::first_period_stub_type(issue, first, Frequency::SemiAnnual);
        assert_eq!(stub_type, IrregularStubType::LongStub);
    }
